...
associate_measurements <- function(
  ...
  # filter out folders starting "_" and require date-like names
    ...
    for (platform_dir in platform_directories) {
      ...
      if (length(per_platform_meas_long)) {
        ...
        if (nrow(meas_not_in_base)) {
          ...
          log_line(sprintf("      No base match for some measurement keys: %s", ex))
        }
        if (nrow(base_not_in_meas)) {
          ex <- paste(head(sprintf("[%s, %s]", base_not_in_meas$FileName_key, base_not_in_meas$EGNO_key), 5), collapse = "; ")
          log_line(sprintf("      Base rows with no pixel measurements: %s", ex))
        }
      ...
      log_line(sprintf("      Updated %s — EGNO CSVs=%s; pixel rows=%s; unique keys=%s; matched base rows=%s",
                       basename(pm_file), csv_count, pix_rows, n_meas_keys, matched_rows))
